...
void Shader::setShaderSrc(std::string vertSrcPath, std::string fragSrcPath)
{
    this->m_vertSrcPath = vertSrcPath;
    this->m_fragSrcPath = fragSrcPath;
...
void Shader::setShaderCode(const char* vertCode, const char* fragCode)
{
    compileShader(vertCode, GL_VERTEX_SHADER);
    compileShader(fragCode, GL_FRAGMENT_SHADER);
...
{
    std::stringstream vertSrcStream = this->getSource(m_vertSrcPath);
    compileShader(vertSrcStream.str().c_str(), GL_VERTEX_SHADER);

    std::stringstream fragSrcStream = this->getSource(m_fragSrcPath);
    compileShader(fragSrcStream.str().c_str(), GL_FRAGMENT_SHADER);
...
void Shader::setUBOBinding(const std::string& uniformBlockName, const int& bindingPoint)
{
